          ...
          <tbody class="scroll-y-auto fontMedium text-quinary">
            <tr v-for="(item, index) in curvesFiltered" :key="index">
              <td>
                ...
                <template v-else>
                  <input v-model="singleSelectedChannels" type="radio" :value="item.ChannelInfo?.ChannelId" />
                </template>
              </td>
              <td>
  ...
  export default defineComponent({
    ...
    data() {
      return {
        ...
        singleSelectedChannels: this.modelValue[0]?.id || (0 as number),
